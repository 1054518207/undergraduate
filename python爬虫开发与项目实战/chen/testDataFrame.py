    ...
    if excelFileName is None:
        ...
        for item in all_files:
            if item.endswith("xlsx") and item.__contains__(str(excelFileName)):
                ...
                break
        if fileName is None:
            raise RuntimeError("找不到{}文档".format(excelFileName))
        return fileName
...
if __name__ == '__main__':
    ...
    print("准确率为：{}".format(metrics.accuracy_score(trueLabel,predictLabel)))
    dfPredict = pd.DataFrame(data={"num":dfTest['User'],"trueLabel":dfTest['level'],"predictLabel":predictLabel})
