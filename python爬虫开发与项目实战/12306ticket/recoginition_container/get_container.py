...
class RecognitionContainer(object):
    ...
    def __upload_pic__(self, img, filex, url):
        '''
        上传图片，得到图片地址
        :param img: Image
        :param filex: 
        ...
        :return: 
        '''
        img.save(self.base_img_url + "/query_temp_img" + filex + ".png")
        raw = open(self.base_img_url + "/query_temp_img" + filex + ".png", "rb").read()
        files = {
            'fileheight': "0",
        ...
        resp = requests.post(url, files=files, headers=self.headers, verify=False)
        ...
        for x_ in range(0, 2):
            for y_ in range(0, 4):
                ...
                if x_ == 0:
                    ...
                    # 得到查询地址
                query_url = self.__upload_pic__(imgs.crop(box), str(x_) + str(y_), self.url)
                # 进行查询，返回结果
